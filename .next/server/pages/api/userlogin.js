...
const userLoginSchema = (0,yup__WEBPACK_IMPORTED_MODULE_0__.object)().shape({
  ...
  password: (0,yup__WEBPACK_IMPORTED_MODULE_0__.string)().required().min(4).max(25)
      ...
      findUser = async mobile_no => {
  return await _functionalities_DB_prismaInstance__WEBPACK_IMPORTED_MODULE_1__/* .prisma.user.findUnique */ ._.user.findUnique({
    where: {
      mobile_no
    }
  });
...
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = (async (req, res) => {
  ...
  try {
    ...
    // 	abortEarly: false,
    // })
    const data = (0,_functionalities_mutateObjects__WEBPACK_IMPORTED_MODULE_2__/* .mutateContactNumber */ .D)(req.body.data, true);
    const user = await findUser(data["mobile_no"]);
    console.log(user, data); // user existence

    if (user == null) return res.status(200).json({
      user: "User not found! Try Sign Up."
    }); // credential check

    if (user.password == data.password) // todo redirect user with session
      return res.status(200).json((0,_functionalities_mutateObjects__WEBPACK_IMPORTED_MODULE_2__/* .mutateContactNumber */ .D)(user));else if (user.password != data.password) return res.status(200).json({
...
var __webpack_exports__ = __webpack_require__.X(0, [4394], () => (__webpack_exec__(7396)));
